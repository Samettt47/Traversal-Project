...
namespace TraversalProject
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                x.ClearProviders();
                x.SetMinimumLevel(LogLevel.Debug); // LOG ÝÞLEMÝ NEREDEN ÝTÝBAREN BAÞLASIN( BU STAIRDA KÝ KODUN ANLAMI
            ...
            services.AddIdentity<AppUser, AppRole>().AddEntityFrameworkStores<Context>
                ().AddErrorDescriber<CustomIdentityValidator>().AddTokenProvider<DataProtectorTokenProvider<AppUser>>(TokenOptions.DefaultProvider).AddEntityFrameworkStores<Context>();  // BURRASI DA PROJEYE IDENTÝTY
                                                                                                                                                                                          // EKLEMEMÝZÝ SAÐLAR 
            ...
            //{
            //    config.Filters.Add(new AdminPolicyFilter()); // Bu filtreyi tüm controllerlara uygular
            //}).AddFluentValidation();

            ////Member Policy
            //services.AddControllersWithViews(config =>
            //{
            //    config.Filters.Add(new MemberPolicyFilter()); // Bu filtreyi tüm controllerlara uygular
            //}).AddFluentValidation();

            services.AddMvc(config =>
            {
                var policy = new AuthorizationPolicyBuilder().RequireAuthenticatedUser().Build();
                config.Filters.Add(new AuthorizeFilter(policy));        // bu kodlar PROJE SEVÝYESÝNDE AUTHORÝZATÝON SAÐLAR    
            });
